...
namespace QuickCode.DemoUzeyir.TransactionProcessingModule.Api.Controllers
{
    public partial class FeeTypesController : QuickCodeBaseApiController
    {
        ...
        [HttpGet("get-active/{feeTypeIsActive:bool}")]
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(List<GetActiveResponseDto>))]
        [ProducesResponseType(StatusCodes.Status404NotFound, Type = typeof(string))]
        [ProducesResponseType(StatusCodes.Status400BadRequest, Type = typeof(string))]
        public async Task<IActionResult> GetActiveAsync(bool feeTypeIsActive, int? page, int? size)
        {
            if (page < 1)
            {
            ...
            var response = await service.GetActiveAsync(feeTypeIsActive, page, size);
            if (HandleResponseError(response, logger, "FeeType", $"FeeTypeIsActive: '{feeTypeIsActive}'") is {} responseError)
                return responseError;
            return Ok(response.Value);
        }

        [HttpGet("get-by-code/{feeTypeCode}")]
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(GetByCodeResponseDto))]
        [ProducesResponseType(StatusCodes.Status404NotFound, Type = typeof(string))]
        [ProducesResponseType(StatusCodes.Status400BadRequest, Type = typeof(string))]
        public async Task<IActionResult> GetByCodeAsync(string feeTypeCode)
        {
            var response = await service.GetByCodeAsync(feeTypeCode);
            if (HandleResponseError(response, logger, "FeeType", $"FeeTypeCode: '{feeTypeCode}'") is {} responseError)
                return responseError;
            return Ok(response.Value);
        }

        [HttpPatch("deactivate/{feeTypeId:int}")]
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(int))]
        [ProducesResponseType(StatusCodes.Status400BadRequest, Type = typeof(string))]
        public async Task<IActionResult> DeactivateAsync(int feeTypeId, [FromBody] DeactivateRequestDto updateRequest)
        {
            var response = await service.DeactivateAsync(feeTypeId, updateRequest);
            if (HandleResponseError(response, logger, "FeeType", $"FeeTypeId: '{feeTypeId}'") is {} responseError)
                return responseError;
            return Ok(response.Value);
        }
    }
