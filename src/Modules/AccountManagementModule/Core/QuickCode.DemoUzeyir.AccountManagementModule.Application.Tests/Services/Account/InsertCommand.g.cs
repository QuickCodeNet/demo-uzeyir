...
{
    public class InsertAccountCommandTests : IDisposable
    {
        ...
        public InsertAccountCommandTests()
        {
        ...
        public async Task InsertAsync_Should_Return_Success_When_Valid_Request()
        {
            ...
            var fakeResponse = new RepoResponse<AccountDto>(fakeDto, "Success");
            _repositoryMock.Setup(r => r.InsertAsync(It.IsAny<AccountDto>())).ReturnsAsync(fakeResponse);
            // Act
            var result = await _service.InsertAsync(fakeDto);
            // Assert
            Assert.Equal(ResultCodeSuccess, result.Code);
            Assert.Equal(fakeDto, result.Value);
            _repositoryMock.Verify(r => r.InsertAsync(It.IsAny<AccountDto>()), Times.Once);
        }
        ...
        public async Task InsertAsync_Should_Return_NotFound_When_Repository_Returns_404()
        {
            // Arrange
            var fakeDto = TestDataGenerator.CreateFake<AccountDto>("tr");
            var fakeResponse = new RepoResponse<AccountDto>
            {
