...
using QuickCode.DemoUzeyir.AccountManagementModule.Api.Controllers;
...
using AccountStatus = QuickCode.DemoUzeyir.AccountManagementModule.Domain.Enums.AccountStatus;
using CardStatus = QuickCode.DemoUzeyir.AccountManagementModule.Domain.Enums.CardStatus;
using StatementFormat = QuickCode.DemoUzeyir.AccountManagementModule.Domain.Enums.StatementFormat;

namespace QuickCode.DemoUzeyir.AccountManagementModule.Api.Tests
{
    public class AccountHoldersControllerTests
    {
        private readonly Mock<IAccountHolderService> _serviceMock = new();
        private readonly Mock<ILogger<AccountHoldersController>> _loggerMock = new();
        private readonly AccountHoldersController _controller;
        public AccountHoldersControllerTests()
        {
            _controller = new AccountHoldersController(_serviceMock.Object, null, _loggerMock.Object);
        }
        ...
        {
            // Arrange
            var fakeList = TestDataGenerator.CreateFakes<AccountHolderDto>("tr");
            _serviceMock.Setup(s => s.ListAsync(It.IsAny<int?>(), It.IsAny<int?>())).ReturnsAsync(new Response<List<AccountHolderDto>> { Code = 0, Value = fakeList });
            // Act
        ...
        {
            // Arrange
            _serviceMock.Setup(s => s.ListAsync(It.IsAny<int?>(), It.IsAny<int?>())).ReturnsAsync(new Response<List<AccountHolderDto>> { Code = 1, Message = "Error" });
            // Act
            var result = await _controller.ListAsync(1, 10);
            // Assert
        ...
        public async Task GetItemAsync_Should_Return_Ok_When_Found()
        {
            ...
            _serviceMock.Setup(s => s.GetItemAsync(It.IsAny<int>(), It.IsAny<int>())).ReturnsAsync(new Response<AccountHolderDto> { Code = 0, Value = fakeDto });
            // Act
            var result = await _controller.GetItemAsync(fakeDto.AccountId, fakeDto.CustomerId);
            // Assert
            var okResult = Assert.IsType<OkObjectResult>(result);
        ...
        public async Task GetItemAsync_Should_Return_NotFound_When_404()
        {
            // Arrange
            var fakeDto = TestDataGenerator.CreateFake<AccountHolderDto>("tr");
            _serviceMock.Setup(s => s.GetItemAsync(It.IsAny<int>(), It.IsAny<int>())).ReturnsAsync(new Response<AccountHolderDto> { Code = 404, Message = "Not found" });
            // Act
            var result = await _controller.GetItemAsync(fakeDto.AccountId, fakeDto.CustomerId);
            // Assert
            Assert.IsType<NotFoundObjectResult>(result);
        ...
        public async Task GetItemAsync_Should_Return_BadRequest_When_Fail()
        {
            // Arrange
            var fakeDto = TestDataGenerator.CreateFake<AccountHolderDto>("tr");
            _serviceMock.Setup(s => s.GetItemAsync(It.IsAny<int>(), It.IsAny<int>())).ReturnsAsync(new Response<AccountHolderDto> { Code = 1, Message = "Error" });
            // Act
            var result = await _controller.GetItemAsync(fakeDto.AccountId, fakeDto.CustomerId);
            // Assert
        ...
        public async Task InsertAsync_Should_Return_Created_When_Success()
        {
            // Arrange
            var fakeDto = TestDataGenerator.CreateFake<AccountHolderDto>("tr");
            _serviceMock.Setup(s => s.InsertAsync(It.IsAny<AccountHolderDto>())).ReturnsAsync(new Response<AccountHolderDto> { Code = 0, Value = fakeDto });
            // Act
        ...
        public async Task InsertAsync_Should_Return_BadRequest_When_Fail()
        {
            // Arrange
            var fakeDto = TestDataGenerator.CreateFake<AccountHolderDto>("tr");
            _serviceMock.Setup(s => s.InsertAsync(It.IsAny<AccountHolderDto>())).ReturnsAsync(new Response<AccountHolderDto> { Code = 1, Message = "Insert failed" });
            // Act
            var result = await _controller.InsertAsync(fakeDto);
            // Assert
            Assert.IsType<BadRequestObjectResult>(result);
        ...
        public async Task UpdateAsync_Should_Return_Ok_When_Success()
        {
            // Arrange
            var fakeDto = TestDataGenerator.CreateFake<AccountHolderDto>("tr");
            _serviceMock.Setup(s => s.UpdateAsync(It.IsAny<int>(), It.IsAny<int>(), It.IsAny<AccountHolderDto>())).ReturnsAsync(new Response<bool> { Code = 0, Value = true });
            // Act
            var result = await _controller.UpdateAsync(fakeDto.AccountId, fakeDto.CustomerId, fakeDto);
            // Assert
            var okResult = Assert.IsType<OkObjectResult>(result);
        ...
        public async Task UpdateAsync_Should_Return_NotFound_When_404()
        {
            // Arrange
            var fakeDto = TestDataGenerator.CreateFake<AccountHolderDto>("tr");
            _serviceMock.Setup(s => s.UpdateAsync(It.IsAny<int>(), It.IsAny<int>(), It.IsAny<AccountHolderDto>())).ReturnsAsync(new Response<bool> { Code = 404, Message = "Not found" });
            // Act
            var result = await _controller.UpdateAsync(fakeDto.AccountId, fakeDto.CustomerId, fakeDto);
            // Assert
            Assert.IsType<NotFoundObjectResult>(result);
        ...
        public async Task UpdateAsync_Should_Return_BadRequest_When_Fail()
        {
            // Arrange
            var fakeDto = TestDataGenerator.CreateFake<AccountHolderDto>("tr");
            _serviceMock.Setup(s => s.UpdateAsync(It.IsAny<int>(), It.IsAny<int>(), It.IsAny<AccountHolderDto>())).ReturnsAsync(new Response<bool> { Code = 1, Message = "Update failed" });
            // Act
            var result = await _controller.UpdateAsync(fakeDto.AccountId, fakeDto.CustomerId, fakeDto);
            // Assert
            Assert.IsType<BadRequestObjectResult>(result);
        ...
        public async Task DeleteAsync_Should_Return_Ok_When_Success()
        {
            ...
            // Act
            var result = await _controller.DeleteAsync(fakeDto.AccountId, fakeDto.CustomerId);
            // Assert
            var okResult = Assert.IsType<OkObjectResult>(result);
            Assert.True((bool)okResult.Value);
        }
